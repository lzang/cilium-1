...
func startSynchronizingServices() {
	...
	switch {
	case version.Capabilities().EndpointSlice && option.Config.EnableK8sEndpointSliceAutoDetection:
		endpointController, connected = endpointsSlicesInit(k8s.Client(), serEps, swgEps)
...
func endpointsSlicesInit(k8sClient kubernetes.Interface, serEps *serializer.FunctionQueue, swgEps *lock.StoppableWaitGroup) (cache.Controller, bool) {
	var (
